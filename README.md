...
Dub a video podcast into another language: caption-free speech timing,
automatic speaker separation, fluent LLM translation, voice-cloned TTS per
speaker, and the original audio kept underneath, deep-ducked.
...
Only process media you have the right to use, and only clone a person's voice
with their authorization. This repository contains no source media,
transcripts, or voice references.

Sortformer supports at most four speakers. Its model card also notes that
accuracy can degrade outside its training domain, including some languages, so
review diarization and the finished dub before publishing it. See
[SECURITY.md](SECURITY.md) for the model-loading and local-cache trust boundary.
